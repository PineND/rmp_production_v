...
function retryOperation(operation, retries, maxRetries, refresh_interval) {
  if (retries < maxRetries) {
    ...
    console.log(`Max retries reached, ${errorMessage.toLowerCase()}`);
  }
}

function modifyLiveTable(retries = 0, maxRetries = 100, refresh_interval = 100) {
  const table_body = document.querySelector("#root > div > div > div._root_szin5_37 > section > div > table > tbody");
  if (table_body) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "checkUrl" && window.location.href.includes('berkeleytime.com/catalog')) {
    modifyLiveTable();
  }
});

if (window.location.href.includes('berkeleytime.com/catalog')) {
  modifyLiveTable();
}